...
--  Changes: account_no (12-digit BIGINT), alphanumeric customer/employee IDs,
--           geo anomaly removed, ip_country removed
-- =============================================================
...
-- customer_id is now VARCHAR (alphanumeric)
-- =============================================================
CREATE TABLE IF NOT EXISTS dim_customer (
    customer_sk     INT          NOT NULL AUTO_INCREMENT,
    customer_id     VARCHAR(20)  NOT NULL,           -- business key (alphanumeric)
...
CREATE TABLE IF NOT EXISTS dim_account (
    ...
    account_no      BIGINT       NOT NULL,           -- 12-digit account number
...
CREATE INDEX idx_da_account_no ON dim_account(account_no);
...
CREATE TABLE IF NOT EXISTS dim_transaction_type (
    ...
    txn_category    VARCHAR(50)  NOT NULL,
...
-- ip_country removed (geo anomaly feature dropped)
-- References use account_no and alphanumeric customer_id via SKs
-- =============================================================
...
-- Structuring / Smurfing candidates
...
SELECT
    ...
    SUM(ft.amount)/COUNT(ft.txn_fact_id) AS avg_txn_amount
...
HAVING
    ...
    AND max_single_amount < 1000000
ORDER BY txn_date DESC;

-- Layering / Rapid movement detection
...
GROUP BY dc_from.customer_id, dc_from.full_name, dc_to.customer_id, dc_to.full_name
HAVING
    hop_count >= 3
...
JOIN (
    SELECT from_customer_sk, AVG(amount) AS recent_avg
    FROM   fact_transaction ft
    JOIN   dim_date d ON ft.date_key = d.date_key
...
JOIN (
    SELECT from_customer_sk, AVG(amount) AS baseline_avg, STDDEV(amount) AS baseline_std
    FROM   fact_transaction ft
    JOIN   dim_date d ON ft.date_key = d.date_key
