...
BEGIN
    ...
    -- Mark as AML-reviewed
    UPDATE Transaction SET is_aml_reviewed = 1 WHERE transaction_id = p_transaction_id;
END$$
...
-- Trigger: After transaction insert → run AML checks
CREATE TRIGGER trg_after_transaction_insert
AFTER INSERT ON Transaction
FOR EACH ROW
BEGIN
    IF NEW.from_account_id IS NOT NULL THEN
        CALL sp_run_aml_checks(NEW.transaction_id);
    END IF;
END$$
