...
--  Run AFTER realistic_seed_data.sql
-- ============================================================

USE finguard_warehouse;

-- Temporarily relax ONLY_FULL_GROUP_BY for the aggregation inserts
...
-- STEP 2: dim_customer
INSERT INTO dim_customer (customer_sk, customer_id, full_name, PAN_number, city, state, age_bucket, risk_tier, is_flagged, effective_from, effective_to, is_current)
SELECT c.customer_id, c.customer_id, c.full_name, c.PAN_number, c.city, b.state,
  CASE WHEN TIMESTAMPDIFF(YEAR,c.DOB,CURDATE())<25 THEN '<25'
       ...
       ELSE '65+' END,
  CASE WHEN c.risk_score>=75 THEN 'CRITICAL'
       WHEN c.risk_score>=50 THEN 'HIGH'
       WHEN c.risk_score>=25 THEN 'MEDIUM'
       ELSE 'LOW' END,
  c.is_flagged, DATE(c.created_at), NULL, 1
FROM finguard_bank.Customer c
JOIN finguard_bank.Branch b ON c.branch_id = b.branch_id;

-- STEP 3: dim_account
INSERT INTO dim_account (account_sk, account_id, account_type, branch_name, city, state, IFSC_code)
SELECT a.account_id, a.account_id, a.account_type, b.branch_name, b.city, b.state, b.IFSC_code
...
SELECT
  ...
  COALESCE((SELECT COUNT(*) FROM finguard_bank.Alert al WHERE al.transaction_id = t.transaction_id),0),
  CASE WHEN a_from.branch_id IS NOT NULL AND a_to.branch_id IS NOT NULL
            AND a_from.branch_id != a_to.branch_id THEN 1 ELSE 0 END
FROM finguard_bank.Transaction t
LEFT JOIN finguard_bank.Account a_from ON t.from_account_id = a_from.account_id
LEFT JOIN finguard_bank.Account a_to   ON t.to_account_id   = a_to.account_id
...
-- STEP 5: fact_alert
INSERT INTO fact_alert (alert_no, customer_sk, txn_fact_id, date_key, alert_type, severity, was_true_positive)
SELECT al.alert_no, al.customer_id, al.transaction_id,
...
INSERT INTO agg_customer_monthly
  (customer_sk, `year_month`, total_outflow, total_inflow, txn_count, distinct_payees, max_single_txn, alert_count, risk_score_end)
SELECT
  a_from.customer_id,
  DATE_FORMAT(t.transaction_timestamp,'%Y-%m'),
  COALESCE(SUM(CASE WHEN t.from_account_id = a_from.account_id THEN t.amount ELSE 0 END),0),
  COALESCE(SUM(CASE WHEN t.to_account_id   = a_from.account_id THEN t.amount ELSE 0 END),0),
  COUNT(DISTINCT t.transaction_id),
  COUNT(DISTINCT t.to_account_id),
  MAX(t.amount),
  COALESCE((SELECT COUNT(*) FROM finguard_bank.Alert al WHERE al.customer_id = a_from.customer_id
    AND DATE_FORMAT(al.alert_timestamp,'%Y-%m')=DATE_FORMAT(t.transaction_timestamp,'%Y-%m')),0),
  c.risk_score
FROM finguard_bank.Transaction t
JOIN finguard_bank.Account a_from ON (t.from_account_id = a_from.account_id OR t.to_account_id = a_from.account_id)
JOIN finguard_bank.Customer c ON a_from.customer_id = c.customer_id
WHERE t.transaction_status = 'COMPLETED'
  AND DATE(t.transaction_timestamp) BETWEEN '2024-09-01' AND '2025-03-15'
GROUP BY a_from.customer_id, DATE_FORMAT(t.transaction_timestamp,'%Y-%m'), c.risk_score
ON DUPLICATE KEY UPDATE
  total_outflow=VALUES(total_outflow), total_inflow=VALUES(total_inflow),
  txn_count=VALUES(txn_count), distinct_payees=VALUES(distinct_payees),
  max_single_txn=VALUES(max_single_txn), alert_count=VALUES(alert_count), risk_score_end=VALUES(risk_score_end);
